...
public class TypeSelectGUI implements Listener {
    public static void open(Player player) {
        ...
        VexText text = new VexText(45, -5, Arrays.asList("§f校准-§6注入数值"), 2);
        ButtonFunction f1 = new ButtonFunction() {
            @Override
            public void run(Player player) {
                EquipSelectGUI.open(player, "突击步枪");
            }
        };
        ...
        ButtonFunction f2 = new ButtonFunction() {
            @Override
            public void run(Player player) {
                EquipSelectGUI.open(player, "精准射手步枪");
            }
        };
        ...
        ButtonFunction f3 = new ButtonFunction() {
            @Override
            public void run(Player player) {
                EquipSelectGUI.open(player, "狙击枪");
            }
        };
        ...
        ButtonFunction f4 = new ButtonFunction() {
            @Override
            public void run(Player player) {
                EquipSelectGUI.open(player, "霰弹枪");
            }
        };
        ...
        ButtonFunction f7 = new ButtonFunction() {
            @Override
            public void run(Player player) {
                EquipSelectGUI.open(player, "冲锋枪");
            }
        };
        ...
        VexButton button_normal = new VexButton("tb_normal", "普通装备", "[local]ISAC/test-button.png", "[local]ISAC/test-button_.png", 70, 210, 64, 45, f13);
